  ...
  return (
    <section className="border-b border-hairline/40 last:border-b-0">
      <button
        ...
        onClick={onToggle}
        aria-expanded={open}
        className={`flex w-full items-baseline gap-2 py-2.5 text-left transition-colors ${open ? "text-cream" : "hover:text-cream"}`}
      >
      ...
      >
        <div className="overflow-hidden" inert={!open}>
          {everOpened && (
            <ul className="rounded-sm bg-cream/[0.03] px-2.5 pb-1.5 pt-0.5">
              {day.rows.map((row) => (
