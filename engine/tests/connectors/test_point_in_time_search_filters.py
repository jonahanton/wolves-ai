...
from wolves.connectors._dates import freshness_range, parse_date, to_end_iso8601, to_iso8601
...
def test_to_end_iso8601_emits_end_of_day():
    assert to_end_iso8601("2026-06-01") == "2026-06-01T23:59:59.999Z"
    assert to_end_iso8601("nonsense") is None
...
async def test_brave_combines_relative_freshness_with_as_of_date():
    captured: list[httpx.Request] = []
    ...
    client = httpx.AsyncClient(transport=httpx.MockTransport(handler))
    brave = BraveClient("key", client=client)
    await brave.search("query", freshness="pd", end_published_date="2026-06-14")
    await client.aclose()

    params = dict(captured[0].url.params)
    assert params["freshness"] == "2026-06-13to2026-06-14"
...
async def test_exa_bounds_results_with_end_published_date():
    ...
    assert body["endPublishedDate"] == "2026-06-01T23:59:59.999Z"


async def test_exa_combines_relative_freshness_with_as_of_date():
    captured: list[httpx.Request] = []
    ...
    client = httpx.AsyncClient(transport=httpx.MockTransport(handler))
    exa = ExaClient("key", client=client)
    await exa.search("query", freshness="pd", end_published_date="2026-06-14")
    await client.aclose()

    body = json.loads(captured[0].content)
    assert body["startPublishedDate"] == "2026-06-13T00:00:00.000Z"
    assert body["endPublishedDate"] == "2026-06-14T23:59:59.999Z"
