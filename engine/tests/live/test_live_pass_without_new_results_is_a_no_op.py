...
async def test_second_pass_with_the_same_results_writes_nothing(tmp_path):
    ...
    assert await live_pass(settings, fixtures=fixtures, n_sims=120, seed=2, forecaster=_forecaster(tmp_path)) is True
    written = sorted(p.name for p in tmp_path.glob("*.json"))

    assert await live_pass(settings, fixtures=fixtures, n_sims=120, seed=2, forecaster=_forecaster(tmp_path)) is False
