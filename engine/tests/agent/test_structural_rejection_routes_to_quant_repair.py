...
from wolves.agent.tools.submission.submit_forecast import _structural_repair_result
...
def test_quant_owned_failure_routes_once_without_spending_a_retry(deps: AgentDeps):
    result = _structural_repair_result(_report("factor_audit_missing_coverage"), deps, artifact_id="mixture-002")
...
def test_same_signature_repeat_falls_through_to_hard_path(deps: AgentDeps):
    ...
    assert _structural_repair_result(report, deps, artifact_id="mixture-002") is not None
    deps.submission.structural_repair_required = False

    assert _structural_repair_result(report, deps, artifact_id="mixture-002") is None
