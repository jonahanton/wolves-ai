...
def test_landing_history_uses_only_snapshots_in_the_selected_archive_payload():
    archive_source = (REPO_ROOT / "web/src/lib/archive/view.ts").read_text(
        encoding="utf-8"
    )
    landing_source = (REPO_ROOT / "web/src/components/archive/archive-landing-page.tsx").read_text(
        encoding="utf-8"
    )
    ...
    assert "fetch(" not in landing_source
